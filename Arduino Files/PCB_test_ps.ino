...
int ana_val[4] = {0,0,0,0};
int n = 0;
int ss_sample_lasttime = 0;
int ss_sample_delay = 250;
...
void loop() {
  ...
  if(millis() >= ss_sample_lasttime + ss_sample_delay){
      ana_val[n] = analogRead(PIN_ANALOG_IN);
      ss_sample_lasttime = millis();
      n++;
  }

  if(ana_val[0] >= ss_threshold && ana_val[1] >= ss_threshold && ana_val[2] >= ss_threshold && ana_val[3] >= ss_threshold){
    bitSet(states, 1);   
